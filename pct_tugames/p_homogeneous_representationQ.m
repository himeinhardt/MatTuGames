function [hrQ hr wmg]=p_homogeneous_representationQ(th,w_vec)
% P_HOMOGENEOUS_REPRESENTATIONQ  checks whether the weighted majority game
...
[mW, ~, wmg]=p_minimal_winning(th,w_vec);
n=length(w_vec);
parfor k=1:n, Clm(:,k)=bitget(mW,k); end
qtvec=Clm*w_vec';
qS=qtvec==th;
hrQ=all(qS);
if hrQ
