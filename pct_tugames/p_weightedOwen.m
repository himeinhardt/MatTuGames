function ow_vl=p_weightedOwen(v,cs)
% P_WEIGHTEDOWEN computes the weighted Owen value w.r.t. 
% a priori unions cs using Matlab's PCT. 
% The weights will be computed from cs.
%
% Usage: ow_vl=p_weightedOwen(v,cs)
%
% Define variables:
%  output:
%  ow_vl    -- The weihgted Owen value.
%
%  input:
...
wgs=(csm*ones(n,1))';
sh_cw=p_weightedShapley(cw,wgs);
ow_vl=zeros(1,n);
iow=cell(1,lcs);
idx=cell(1,lcs);
  parfor jj=1:lcs
