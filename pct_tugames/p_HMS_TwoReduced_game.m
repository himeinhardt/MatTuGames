function [v_t,sS,PlyMat2]=p_HMS_TwoReduced_game(v,x,str,tol)
% P_HMS_TWOREDUCED_GAME computes from (v,x) all Hart/Mas-Colell 
...
parfor k=1:lS2
  [v1_t{1,k} v2_t{1,k} v3_t{1,k}]=p_hms_red_game(v,x,sS(k),n,str,tol,lmQ);
end
...
%---------------------------------
function [vt subg_sh T]=p_hms_red_game(v,x,S,n,str,tol,lmQ)
   ...
   if crQ==1	   
      ...
      if bcQ==1
         ...
         if convex_gameQ(subg{k})
           subg_sh{k}=FindHMConsElm(subg{k},y,TorcS(k),S,n,lmQ); % try to get weak HM-consistent core element.            
         else
