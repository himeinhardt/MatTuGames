function [WRGP WRGPC]=p_WeakReduced_game_propertyQ(v,x,str,tol)
...
%
%
% Usage: [WRGP WRGPC]=p_WeakReduced_game_propertyQ(v,x,str,tol)
%
% Define variables:
%  output: Fields
...
if strcmp(str,'SHAP')
%vS=cell(2,N);
  [vSa,sS,PlyMat]=p_HMS_TwoReduced_game(v,x,'SHAP');
  vS=vSa{:,1};
  clear vSa;  
elseif strcmp(str,'HMS_PK')
  [vSa,sS,PlyMat]=p_HMS_TwoReduced_game(v,x,'PRK');
  vS=vSa{:,1};
  clear vSa;  
elseif strcmp(str,'HMS_PN')
  [vSa,sS,PlyMat]=p_HMS_TwoReduced_game(v,x,'PRN');
  vS=vSa{:,1};
  clear vSa;  
elseif strcmp(str,'HMS_CORE')
  [vSa,sS,PlyMat]=p_HMS_TwoReduced_game(v,x,'CORE');
  vS=vSa{:,1};
  clear vSa;  
else
  [vSa,sS,PlyMat]=p_DM_TwoReduced_game(v,x);
  vS=vSa{:,1};
  clear vSa;  
end
...
% the pre-nucleolus and and the pre-kernel. 
  ...
  elseif strcmp(str,'MODIC')
   if length(vS{k})==1
     ...
     try
       sol{k}=msk_modiclus(vS{k}); % using moske pre-nucleolus function. 
     catch
...
if strcmp(str,'SHAP')
   sol{nlS}=ShapleyValue(v);
   rgpq_sol{nlS}=abs(sol{nlS}-x)<tol;
   rgpq(nlS)=all(rgpq_sol{nlS});
elseif strcmp(str,'PRK')
  rgpq(nlS)=PrekernelQ(v,x);
elseif strcmp(str,'POPK')
  rgpq(nlS)=positivePrekernelQ(v,x);
elseif strcmp(str,'MPRK')
  rgpq(nlS)=ModPrekernelQ(v,x);
elseif strcmp(str,'PMPRK')
  rgpq(nlS)=PModPrekernelQ(v,x);
elseif strcmp(str,'PRN')
   try
     sol{nlS}=Prenucl(v,x);
   catch
     sol{nlS}=PreNucl2(v,x); % use a third party solver instead!
   end
   rgpq_sol{nlS}=abs(sol{nlS}-x)<tol;
   rgpq(nlS)=all(rgpq_sol{nlS});
elseif strcmp(str,'MODIC')
   try
     sol{nlS}=msk_modiclus(v);
   catch
     sol{nlS}=Modiclus(v); % use a third party solver instead!
   end
   rgpq_sol{nlS}=abs(sol{nlS}-x)<tol;
   rgpq(nlS)=all(rgpq_sol{nlS});
elseif strcmp(str,'HMS_PK')
...
elseif strcmp(str,'CORE')
    ...
    if crQ==1
       rgpq(nlS)=belongToCoreQ(v,x,'rat',tol);
    else
      rgpq(nlS)=false;
    end
elseif strcmp(str,'HMS_CORE')
