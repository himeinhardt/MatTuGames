function [ACDGP ACDGPC]=p_Anti_Converse_DGP_Q(v,x,str,tol)
% P_ANTI_CONVERSE_DGP_Q checks whether an imputation x satisfies the anti-converse derived game property 
% (CDGP), that is, a modified converse reduced game property (CRGP) using Matlab's PCT.
%
...
%
% Usage [ACDGP ACDGPC]=p_Anti_Converse_DGP(v,x,str,tol)
% Define variables:
%  output: Fields
...
% Checking now whether the imputation solves the anti-derived game for 
...
sV_x{k}=x;
 if strcmp(str,'SHAP')
   vS{k}=HMS_Anti_Derived_game(v,x,cl2(k)); %Hart-MasColell anti-derived game.
   ...
   vS{k}=Anti_DerivedGame(v,x,cl2(k)); % Davis-Maschler anti-derived game.
   stdsol{k}=StandardSolution(vS{k}); % solution x restricted to S.
   rS{k}=PlyMat2(k,:);
   sV_x{k}(rS{k})=stdsol{k}; % extension to (x,x_N\S).
   if strcmp(str,'MAPRK')
     crgpQ(k)=Anti_ModPrekernelQ(v,sV_x{k});
   elseif strcmp(str,'PMAPRK')
     crgpQ(k)=Anti_PModPrekernelQ(v,sV_x{k});
   elseif strcmp(str,'MODIC')
     crgpQ(k)=modiclusQ(v,sV_x{k});
   elseif strcmp(str,'APRK')
     crgpQ(k)=Anti_PrekernelQ(v,sV_x{k});
