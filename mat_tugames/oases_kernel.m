function [x, Lerr, smat, xarr]=oases_kernel(v,x)
% OASES_KERNEL computes from (v,x) a Kernel element using qpOASESmex
% for version 3.2.0 and higher.
% Source: Meinhardt, 2010.
%
%  http://www.kuleuven.be/optec/software/qpOASES
%
%
% Usage: [x Lerr smat xarr] = oases_kernel(v,x)
%
% Define variables:
%  output:
...
%    opts.enableEqualities=true;
    ...
    [x,fval,exitflag,output,lambda] = qpOASES(Q,b,A,vi,ra,lbA,ubA,opts,auxInput);
    if exitflag ~= 0
       x=x';
