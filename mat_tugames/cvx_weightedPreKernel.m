function [x, Lerr, smat, xarr]=cvx_weightedPreKernel(v,pS,x)
% CVX_WEIGHTEDPREKERNEL computes from (v,x) a weighted pre-kernel element using CVX.
...
%
% Usage: [x Lerr smat xarr] = cvx_weightedPreKernel(v,pS,x)
%
% Define variables:
%  output:
...
%  smat     -- Matrix of maximum weignted surpluses.
...
% Calling cvx
    cvx_begin quiet
        cvx_precision('high') % default is faster! 
        variable x(n)
...
%  warning('Prk:No','Probably no pre-kernel found!');
%  x=x';
end
