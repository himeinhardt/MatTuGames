function [WRGP WRGPC]=WeakReduced_game_propertyQ(v,x,str,tol)
...
%
%
% Usage: [WRGP WRGPC]=WeakReduced_game_propertyQ(v,x,str,tol)
%
% Define variables:
%  output: Fields
...
if strcmp(str,'SHAP')
%vS=cell(2,N);
  [vS,sS,PlyMat]=HMS_TwoReduced_game(v,x,'SHAP');
elseif strcmp(str,'HMS_PK')
  [vS,sS,PlyMat]=HMS_TwoReduced_game(v,x,'PRK');
elseif strcmp(str,'HMS_PN')
  [vS,sS,PlyMat]=HMS_TwoReduced_game(v,x,'PRN');
elseif strcmp(str,'HMS_CORE')
  [vS,sS,PlyMat]=HMS_TwoReduced_game(v,x,'CORE');
else
  [vS,sS,PlyMat]=DM_TwoReduced_game(v,x);
end
lS=length(sS);
nlS=lS+1;
rgpq=false(1,nlS);
