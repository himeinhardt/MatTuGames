function CMAT=getCOV3(obs,n,seed)
...
rdata=zeros(obs,n);
for kk=1:n  %% higher return is related to higher risk (standard assumption)
    %% a/2+n-kk decreases std for asset kk w.r.t. kk-1.
    rdata(:,kk)=(a/2+n-kk).*randn(obs,1) + av(kk);
end
Sig=cov(rdata);
mu=mean(rdata);
R=mvnrnd(mu,Sig,obs);
