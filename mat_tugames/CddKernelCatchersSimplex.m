...
[~,~,imp_vol,Pip]=CddImputationSimplexVertices(v);
if core_sol==1
  [~,~,cr_vol,Pv]=CddCoreSimplexVertices(v);
end
[~,~,rs_vol,Prea]=CddReasonableSetSimplexVertices(v);
[~,~,prs_vol,Pprea]=CddUpperSetSimplexVertices(v);
sma=smallest_amount(v);
lwseQ=sum(sma)>v(N);
if lwseQ==1
else
  [~,~,lws_vol,Plws]=CddLowerSetSimplexVertices(v);
end
