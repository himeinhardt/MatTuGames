function [CRGP CRGPC]=Converse_CmpConsistencyQ(v,x,str,tol)
% CONVERSE_CMPCONSISTENCYQ checks whether an imputation x satisfies the CCmpCons, that is,
% the converse complement consistency property (converse complement consistency).
%
% Usage [CRGP CRGPC]=Converse_CmpConsistencyQ(v,x,str,tol)
% Define variables:
%  output: Fields
...
sV_x{1,k}=x;
 if strcmp(str,'SHAP')
   ...
   vS{1,k}=CmpRedGame(v,x,cl2(k)); % complement reduced game.
   stdsol{1,k}=StandardSolution(vS{1,k}); % solution x restricted to S.
   rS{k}=PlyMat2(k,:);
   sV_x{1,k}(rS{k})=stdsol{1,k}; % extension to (x,x_N\S).
   if strcmp(str,'PRK')
