function [CDGP CDGPC]=Converse_DGP_Q(v,x,str,tol)
...
%
% Usage [CDGP CDGPC]=Converse_DGP(v,x,str,tol)
% Define variables:
%  output: Fields
...
sV_x{1,k}=x;
 if strcmp(str,'SHAP')
   vS{1,k}=HMS_Derived_game(v,x,cl2(k)); %Hart-MasColell derived game.
   ...
   vS{1,k}=DerivedGame(v,x,cl2(k)); % Davis-Maschler derived game.
   stdsol{1,k}=StandardSolution(vS{1,k}); % solution x restricted to S.
   rS{k}=PlyMat2(k,:);
   sV_x{1,k}(rS{k})=stdsol{1,k}; % extension to (x,x_N\S).
   if strcmp(str,'MPRK')
     crgpQ(k)=ModPrekernelQ(v,sV_x{1,k});
   elseif strcmp(str,'PMPRK')
     crgpQ(k)=PModPrekernelQ(v,sV_x{1,k});
