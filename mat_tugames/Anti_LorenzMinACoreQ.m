function ALMQ=Anti_LorenzMinACoreQ(v,x,tol)
...
%
% Usage: ALMQ=Anti_LorenzMinACoreQ(v,x,tol)
%
% Define variables:
...
try
   crQ=CddAntiCoreQ(v,tol);
catch
   crQ=anti_coreQ(v,tol);
end	

if crQ==1	
   bcQ=belongToAntiCoreQ(v,x,'rat',tol);    
   crv=CddAntiCoreVertices(v,'',tol);
   s1=size(crv,1);
   lmQ=false(1,s1);
   dzQ=false(1,s1);
   for k=1:s1
       LD = Anti_LorenzDom(v,x,crv(k,:),tol);
       lmQ(k)=LD.ldQ;
