function [x, Lerr, smat, xarr]=msk_AntiPreKernel(v,x)
...
%
% Usage: [x Lerr smat xarr] = msk_prekernel(v,x)
%
% Define variables:
%  output:
...
[x, Lerr, smat, xarr]=computeAPrk(v,x,smc,0,mnQ);
smat=tril(smat,-1)+triu(smat,1);

% Main function to compute a
% pre-kernel element.
%-----------------------------
function [x, Lerr, smat, xarr]=computeAPrk(v,x,smc,slv,mnQ)
% 
%  output:  -- as above.
%
%  input:
...
%param.MSK_IPAR_OPTIMIZER = 1;  % Using interior-point optimizer (MSK 7).
...
% Calling conic programming solver.
    ...
    if strcmp(sol.itr.solsta,'OPTIMAL') ~= 1
       x=sol.itr.xx(1:n)';
       warning('ker:No','Probably no anti pre-kernel point found!');
       break;
    end
    x=sol.itr.xx(1:n);
...
if cnt==CNT, % should trigger errors ....
  if slv==0 && smc==1
       ...
       [x, Lerr, smat, xarr]=computeAPrk(v,x,0,slv,mnQ);
  elseif slv==0 && smc==0
       ...
       [x, Lerr, smat, xarr]=computeAPrk(v,x,smc,1,mnQ);
  elseif slv==1 && smc==0
       ...
       [x, Lerr, smat, xarr]=computeAPrk(v,x',1,1,mnQ);
  else
       x=x';
