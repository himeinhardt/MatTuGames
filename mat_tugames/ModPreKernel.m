function [x, Lerr, smat, xarr]=ModPreKernel(v,x)
...
%
%
% Usage: [x Lerr smat xarr]=ModPreKernel(v,x)
%
% Define variables:
%  output:
%  x        -- A modified Pre-Kernel element (output)
...
x=x';
dv=dual_game(v);
[x, Lerr, smat, xarr]=computePrk(v,x,smc,0,mnQ,dv);
smat=tril(smat,-1)+triu(smat,1);

% Main function to compute a
% pre-kernel element.
%-----------------------------
function [x, Lerr, smat, xarr]=computePrk(v,x,smc,slv,mnQ,dv)
% 
%  output:  -- as above.
%
%  input:
...
while cnt<CNT  
    cnt=cnt+1;
    [A, smat, pv]=effCoalitions(v,x,smc,cnt,dv,[]);
    if isempty(A) %% is empty if truncating of the data array has failed.
       [A, smat]=effCoalitions(v,x,smc,cnt,dv,pv);
    end
    if isempty(A)
       msg00='Probably No Modified Pre-Kernel Element found!.';
       warning('MPrK:ChangCard',msg00);
       x=-inf(1,n);
       Lerr=[];
       smat=[];
       xarr=[];
...
if cnt==CNT % should trigger errors ....
  ...
  elseif slv==1 && smc==0 
       ...
       [x, Lerr, smat, xarr]=computePrk(v,x,1,1,mnQ,dv);
  elseif slv==1 && smc==1
       if mnQ==1 && n < 8 
       ...
       [x, Lerr, smat, xarr]=computePrk(v,x,2,1,mnQ,dv);
  else
       x=x';
...
%
%--------------
function [A, smat, pv]=effCoalitions(v,x,smc,cnt,dv,pv)
