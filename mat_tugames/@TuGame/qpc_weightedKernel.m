function [x, Lerr, smat, xarr]=qpc_weightedKernel(clv,pS,x)
% QPC_WEIGHTEDKERNEL computes from (v,pS,x) a weighted kernel element using qpas.mex.
% Source: Meinhardt, 2010.
%
% QPC-Solver: http://sigpromu.org/quadprog/index.html 
%
% Usage: [x Lerr smat xarr] = qpc_weightedKernel(clv,pS,x)
%
% Define variables:
%  output:
...
%  smat     -- Matrix of maximum weightd surpluses.
...
% Calling quadratic programming solver.
    [x,err2,lm]  = qpas(Q,b,[],[],E(m,:),a(m),vi,ra);
    if err2 ~= 0 
       x=x';
