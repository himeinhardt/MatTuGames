function [x, Lerr, smat, xarr]=cp_prekernel(clv,x)
% CP_PREKERNEL computes from (clv,x) a pre-kernel element using the cone programming solver of
% MATLAB's Optimization Toolbox.
% Source: Meinhardt, 2013.
%
% CONE-Solver: coneprog (R2020b)   
%
% Usage: [x Lerr smat xarr] = clv.cp_prekernel(x)
%
% Define variables:
%  output:
...
%% Conic Optimization
%% Setting Options
% opts = optimoptions("coneprog",'Display','iter',"OptimalityTolerance",1e-8,"ConstraintTolerance",1e-8,"LinearSolver","augmented","Display","off");
opts = optimoptions("coneprog",'Display','iter',"OptimalityTolerance",1e-14,"ConstraintTolerance",1e-14,"LinearSolver","normal","Display","off");
...
%% Calling conic programming solver.
    ...
    if exitflag ~= -7 || exitflag == 1
       x=u(1:(end-1))';
%       warning('prker:No','Probably no pre-kernel point found!');
       break;
    end
    x=u(1:(end-1));
%    x'
