function [ACDGP ACDGPC]=p_Anti_Converse_DGP_Q(clv,x,str,tol)
...
%
% Usage [ACDGP ACDGPC]=clv.p_Anti_Converse_DGP(x,str,tol)
% Define variables:
%  output: Fields
...
sV_x{k}=x;
 if strcmp(str,'SHAP')
   vS{k}=clv.HMS_Anti_Derived_game(x,cl2(k)); %Hart-MasColell anti-derived game.
   ...
   vS{k}=clv.Anti_DerivedGame(x,cl2(k)); % Davis-Maschler anti-derived game.
   stdsol{k}=StandardSolution(vS{k}); % solution x restricted to S.
   rS{k}=PlyMat2(k,:);
   sV_x{k}(rS{k})=stdsol{k}; % extension to (x,x_N\S).
   if strcmp(str,'MAPRK')
