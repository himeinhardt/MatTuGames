function [bcQ, cmat, S]=weightedAntiB0_balancedCollectionQ(clv,x,pS,tol)
% WEIGHTEDANTIB0_BALANCEDCOLLECTIONQ verifies whether the set of induced
% coalitions is a weighted anti B0_balanced collection. Checking Kohlberg's criterion.
% Requires Matlab's Optimization toolbox (default), otherwise CPLEX.
% Uses now Dual-Simplex (Matlab R2015a).
% 
%
% Usage: bcQ=clv.weightedAntiB0_balancedCollectionQ(x,tol)
% Define variables:
%  output:
...
if nargin < 2
   ...
   if isa(clv,'TuSol');
      ...
      try
        x = clv.weightedAnti_Nucl_llp();
      catch
        x = clv.weightedAnti_Nucl();
      end
   end
   pS='';
elseif nargin<3
...
if bcQ==1
   ...
   while 1
       [cmat,xS,ef]=CheckAntiB0Bal(n,cS{kk});
       if isempty(cS{kk})
          bcQ=false;
          break;
       elseif ef~=1
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [cmat,sol,ef]=CheckAntiB0Bal(n,iS)
% CHECKANTIB0BAL checks weighted B0-balancedness of the collection iS.
%
%
% Define variables:
%  output:
