function [x, Lerr, smat, xarr]=weightedAnti_Kernel(clv,pS,x)
% WEIGHTEDANTI_KERNEL computes from (v,pS,x) a weighted anti-kernel element using
% Matlab's Optimization Toolbox.
% Source: Meinhardt, 2010.
%
% Usage: [x Lerr smat xarr] = weightedAnti_Kernel(clv,pS,x)
%
% Define variables:
%  output:
%  x        -- A weighted anti kernel element (output)
%              (default per capita anti kernel).
...
[x, Lerr, smat, xarr]=computeAkr(v,x,pS,smc,0);
smat=tril(smat,-1)+triu(smat,1);

% Main function to compute a
% pre-kernel element.
%-----------------------------
function [x, Lerr, smat, xarr]=computeAkr(v,x,pS,smc,slv)
% 
%  output:  -- as above.
%
%  input:
...
%    opts = optimset(opts,'MSK_DPAR_INTPNT_TOL_DFEAS',1.0000e-14,'MSK_DPAR_INTPNT_TOL_PFEAS',1.0000e-14,'MSK_DPAR_INTPNT_TOL_MU_RED',1.0000e-14,'MSK_DPAR_INTPNT_TOL_REL_GAP',1.0000e-14);
    opts = optimset('Algorithm','interior-point-convex','Display','off','TolFun',1e-10);
...
if cnt==CNT, % should trigger errors ....
  if slv==0 && smc==1
       ...
       [x, Lerr, smat, xarr]=computeAkr(v,x',pS,0,slv);
  else
       if irQ
          smat=tril(smat,-1)+triu(smat,1);
          krm=smat-smat';
