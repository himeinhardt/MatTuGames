function [x, Lerr, smat, xarr]=p_PModPreKernel(clv,x)
% P_PREKERNEL computes from (v,x) a proper modified pre-kernel element using Matlab's PCT.
%
...
%
%
% Usage: [x Lerr smat xarr]=clv.p_PModPreKernel(x)
%
% Define variables:
%  output:
%  x        -- Proper modified pre-Kernel element (output)
...
while cnt<CNT  
    cnt=cnt+1;
    [A, smat,pv]=effCoalitions(v,x,smc,cnt,me,[]);
    if isempty(A) %% is empty if truncating of the data array has failed.
       [A, smat]=effCoalitions(v,x,smc,cnt,me,pv);
    end
    if isempty(A)
       x=-inf(1,n);
       Lerr=[];
...
%--------------
function [A, smat, pv]=effCoalitions(v,x,smc,cnt,me,pv)
