function bcQ=weightedBalancedCollectionQ(clv,x,pS,tol)
% WEIGHTEDBALANCEDCOLLECTIONQ verifies whether the set of induced
% coalitions is a weighted balanced collection. Checking Kohlberg's criterion.
% Requires Matlab's Optimization toolbox (default), otherwise CPLEX.
% Uses now Dual-Simplex (Matlab R2015a).
% 
%
% Usage: bcQ=clv.weightedBalancedCollectionQ(x,pS,tol)
% Define variables:
%  output:
...
if nargin < 2
   ...
   if isa(clv,'TuSol');
      ...
      try
        x = clv.weightedPreNucl_llp();
      catch
        x = clv.weightedPreNucl();
      end
   end
   pS='';
elseif nargin<3
