function [bcQ, cmat, S]=cs_Anti_Weak_balancedCollectionQ(clv,cs,x,tol)
% CS_ANTI_WEAK_BALANCEDCOLLECTIONQ verifies whether the set of induced
% coalitions is a weak_balanced collection w.r.t. the coalition structure cs. 
...
% 
%
% Usage: bcQ=clv.cs Anti_Weak_balancedCollectionQ(cs,x,tol)
% Define variables:
%  output:
...
warning('off','all');
while 1
   [cmat,xS,ef]=cs_AntiWeakCheckB0Bal(n,S,b0,cs);
   if isempty(S)
      bcQ=false;
      warning('on','all');
      break;
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [cmat,sol,ef]=cs_AntiWeakCheckB0Bal(n,iS,b0,cs)
