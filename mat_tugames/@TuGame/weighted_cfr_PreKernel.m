function [x, Lerr, smat, xarr]=weighted_cfr_PreKernel(clv,F,pS,x)
%WEIGHTED_CFR_PREKERNEL computes from (v,x) a weighted pre-kernel element with coalition formation restrictions.
% Source: Meinhardt, 2010.
%
% Usage: [x Lerr smat xarr]=weighted_cfr_PreKernel(v,F,x)
%
% Define variables:
%  output:
%  x        -- Weighted pre-Kernel element (output) of game vF.
...
[x, Lerr, smat, xarr]=wcfr_computePrk(v,x,pS,smc,0,mnQ,F);
smat=tril(smat,-1)+triu(smat,1);

% Main function to compute a
% pre-kernel element.
%-----------------------------
function [x, Lerr, smat, xarr]=wcfr_computePrk(v,x,pS,smc,slv,mnQ,F)
% 
%  output:  -- as above.
%
%  input:
...
while cnt<CNT  
    cnt=cnt+1;
    [A, smat]=wcfr_effCoalitions(v,x,smc,cnt,pS,F);
    upe=tril(upe,-1);
    etr12=A';
    ec12=etr12(upe)';
    ec21=A(upe)';
...
if cnt==CNT, % should trigger errors ....
  ...
  elseif slv==1 && smc==0 
       ...
       [x, Lerr, smat, xarr]=wcfr_computePrk(v,x,pS,1,1,mnQ,F);
  elseif slv==1 && smc==1
       if mnQ==1 && n < 8 
       ...
       [x, Lerr, smat, xarr]=wcfr_computePrk(v,x,pS,2,1,mnQ,F);
  else
       x=x';
...
%
%--------------
function [A, smat]=wcfr_effCoalitions(v,x,smc,cnt,pS,F)
