function [x, Lerr, smat, xarr]=cfr_PreKernel(clv,F,x)
%CFR_PREKERNEL computes from (v,x) a pre-kernel element with coalition formation restrictions.
% Source: Meinhardt, 2010.
%
% Usage: [x Lerr smat xarr]=clv.fr_PreKernel(F,x)
%
% Define variables:
%  output:
%  x        -- Pre-Kernel element (output) of game vF.
...
[x, Lerr, smat, xarr]=cfr_computePrk(v,x,smc,0,mnQ,F);
smat=tril(smat,-1)+triu(smat,1);

% Main function to compute a
% pre-kernel element.
%-----------------------------
function [x, Lerr, smat, xarr]=cfr_computePrk(v,x,smc,slv,mnQ,F)
% 
%  output:  -- as above.
%
%  input:
...
if cnt==CNT, % should trigger errors ....
  ...
  elseif slv==1 && smc==0 
       ...
       [x, Lerr, smat, xarr]=cfr_computePrk(v,x,1,1,mnQ,F);
  elseif slv==1 && smc==1
       if mnQ==1 && n < 8 
       ...
       [x, Lerr, smat, xarr]=cfr_computePrk(v,x,2,1,mnQ,F);
  else
       x=x';
