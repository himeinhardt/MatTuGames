function [x, Lerr, smat, xarr]=p_ols_prekernel(clv,x)
% P_OLS_PREKERNEL computes from (v,x) a pre-kernel element using Matlab's PCT.
% Source: Meinhardt, 2010.
%
% Usage: [x Lerr smat xarr]=p_ols_prekernel(clv,x)
%
% Define variables:
%  output:
...
while cnt<CNT  
    cnt=cnt+1;
    [A, smat]=effCoalitions(v,x,smc,gt);
    upe=tril(upe,-1);
    etr12=A';
    ec12=etr12(upe)';
    ec21=A(upe)';
