function [CDGP CDGPC]=p_Converse_DGP_Q(clv,x,str,tol)
...
%
% Usage [CDGP CDGPC]=clv.p_Converse_DGP(x,str,tol)
% Define variables:
%  output: Fields
...
sV_x{k}=x;
 if strcmp(str,'SHAP')
   vS{k}=clv.HMS_Derived_game(x,cl2(k)); %Hart-MasColell derived game.
   ...
   vS{k}=clv.DerivedGame(x,cl2(k)); % Davis-Maschler derived game.
   stdsol{k}=StandardSolution(vS{k}); % solution x restricted to S.
   rS{k}=PlyMat2(k,:);
   sV_x{k}(rS{k})=stdsol{k}; % extension to (x,x_N\S).
   if strcmp(str,'MPRK')
     crgpQ(k)=clv.ModPrekernelQ(sV_x{k});
   elseif strcmp(str,'PMPRK')
     crgpQ(k)=clv.PModPrekernelQ(sV_x{k});
   elseif strcmp(str,'MODIC')
     crgpQ(k)=clv.modiclusQ(sV_x{k});
   elseif strcmp(str,'PRK')
     crgpQ(k)=clv.PrekernelQ(sV_x{k});
