function [x, Lerr, smat, xarr]=weightedAnti_PreKernel(clv,pS,x)
% WEIGHTEDANTI_PREKERNEL computes from (v,pS,x) a weighted anti-pre-kernel element.
% Source: Meinhardt, 2010.
%         Funaki and Meinhardt, 2006.
%
% Usage: [x Lerr smat xarr]=weightedAnti_PreKernel(clv,pS,x)
%
% Define variables:
%  output:
...
[x, Lerr, smat, xarr]=computeAntiPrk(v,x,pS,smc,0,gt);
smat=tril(smat,-1)+triu(smat,1);

% Main function to compute a
% pre-kernel element.
%-----------------------------
function [x, Lerr, smat, xarr]=computeAntiPrk(v,x,pS,smc,slv,gt)
% 
%  output:  -- as above.
%
%  input:
