function [bcQ, cmat, iS]=weightedAntiBalancedCollectionQ(clv,x,pS,tol)
% WEIGHTEDANTIBALANCEDCOLLECTIONQ verifies whether the set of induced
% coalitions is a weighted anti balanced collection. Checking Kohlberg's criterion.
% Requires Matlab's Optimization toolbox (default), otherwise CPLEX.
% Uses now Dual-Simplex (Matlab R2015a).
% 
%
% Usage: bcQ=clv.weightedAntiBalancedCollectionQ(x,pS,tol)
% Define variables:
%  output:
...
if nargin < 2
   ...
   if isa(clv,'TuSol');
      ...
      try
        x = clv.weightedAnti_PreNucl_llp();
      catch
        x = clv.weightedAnti_PreNucl();
      end
   end
   pS='';
elseif nargin<3
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [cmat,sol,ef]=WhgsCheckBal(n,iS)
