function [x, Lerr, smat, xarr]=p_Anti_ModPreKernel(clv,x)
% P_ANTI_MODPREKERNEL computes from (v,x) a modified anti-pre-kernel element using Matlab's PCT.
%
...
%
%
% Usage: [x Lerr smat xarr]=clv.p_Anti_ModPreKernel(x)
%
% Define variables:
%  output:
%  x        -- Modified anti-pre-Kernel element (output)
...
[x, Lerr, smat, xarr]=computeAPrk(v,x',smc,0,mnQ,me,dv);
smat=tril(smat,-1)+triu(smat,1);

% Main function to compute a
% pre-kernel element.
%-----------------------------
function [x, Lerr, smat, xarr]=computeAPrk(v,x,smc,slv,mnQ,me,dv)
% 
%  output:  -- as above.
%
%  input:
...
while cnt<CNT  
    cnt=cnt+1;
    [A, smat, pv]=AeffCoalitions(v,x,smc,cnt,me,dv,[]);
    if isempty(A) %% is empty if truncating of the data array has failed.
       [A, smat]=AeffCoalitions(v,x,smc,cnt,me,dv,pv);
    end
    if isempty(A)
       x=-inf(1,n);
       Lerr=[];
...
if cnt==CNT, % should trigger errors ....
  if slv==0 && smc==1
       ...
       [x, Lerr, smat, xarr]=computeAPrk(v,x,0,slv,mnQ,me,dv);
  elseif slv==0 && smc==0 
...
%--------------
function [A, smat, pv]=AeffCoalitions(v,x,smc,cnt,me,dv,pv)
