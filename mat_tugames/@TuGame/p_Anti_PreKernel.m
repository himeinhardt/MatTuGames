function [x, Lerr, smat, xarr]=p_Anti_PreKernel(clv,x)
...
%
% Usage: [x Lerr smat xarr]=p_Anti_PreKernel(clv,x)
%
% Define variables:
%  output:
...
[x, Lerr, smat, xarr]=computeAntiPrk(v,x',smc,0,gt,mnQ);
smat=tril(smat,-1)+triu(smat,1);

% Main function to compute a
% pre-kernel element.
%-----------------------------
function [x, Lerr, smat, xarr]=computeAntiPrk(v,x,smc,slv,gt,mnQ)
% 
%  output:  -- as above.
%
%  input:
...
while cnt<CNT  
    cnt=cnt+1;
    [A, smat]=effCoalitions(v,x,smc,cnt,gt);
    upe=tril(upe,-1);
    etr12=A';
    ec12=etr12(upe)';
    ec21=A(upe)';
