function [x, Lerr, smat, xarr]=cs_Kernel(clv,cs,x)
% CS_KERNEL computes from (v,x) a Kernel element from a coalition structure 
% using Matlab's Optimization Toolbox.
%
% Usage: [x Lerr smat xarr] = clv.cs_Kernel(cs,x)
%
% Define variables:
%  output:
...
% Setting optins
    ...
    [x,fval,exitflag,output,lambda] = quadprog(Q,b,[],[],Em,am,vi,ra,x,opts);
    if exitflag ~= 1
       x=x';
