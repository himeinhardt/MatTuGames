function [x, Lerr, smat, xarr]=weightedPreKernel(clv,pS,x)
% WEIGTHEDPREKERNEL computes from (v,x) a weighted pre-kernel element.
% Source: Meinhardt, 2014.
%
% Usage: [x Lerr smat xarr]=weightedPreKernel(v,pS,x)
% Define variables:
%  output:
...
[x, Lerr, smat, xarr]=computePrk(v,x,pS,smc,0,mnQ,gt);
smat=tril(smat,-1)+triu(smat,1);

% Main function to compute a
% pre-kernel element.
%-----------------------------
function [x, Lerr, smat, xarr]=computePrk(v,x,pS,smc,slv,mnQ,gt)
% 
%  output:  -- as above.
%
%  input:
...
if cnt==CNT, % should trigger errors ....
  if slv==0 && smc==1
       ...
       [x, Lerr, smat, xarr]=computePrk(v,x',pS,0,slv,mnQ,gt);
  elseif slv==0 && smc==0 
       msg02='No Pre-Kernel Element found. Changing the Solver.';
       warning('PrK:ChangSolv',msg02);
       if mnQ==1; x=2*x; end  
       [x, Lerr, smat, xarr]=computePrk(v,x',pS,smc,1,mnQ,gt);
  elseif slv==1 && smc==0 
       ...
       [x, Lerr, smat, xarr]=computePrk(v,x',pS,1,1,mnQ,gt);
  elseif slv==1 && smc==1
       x=(v(N)/n)*ones(1,n);
       msg01='No Pre-Kernel Element found. Changing to Start Value.';
       warning('PrK:StartVal',msg01);
       [x, Lerr, smat, xarr]=computePrk(v,x,pS,2,1,mnQ,gt);
  else
       x=x';
