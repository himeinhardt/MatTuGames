function [bcQ, cmat, S]=cs_Anti_balancedCollectionQ(clv,cs,x,tol)
% CS_ANTI_BALANCEDCOLLECTIONQ verifies whether the set of induced
% coalitions is an anti balanced collection w.r.t. the coalition structure cs. 
...
% 
%
% Usage: bcQ=clv.cs_Anti_balancedCollectionQ(cs,x,tol)
% Define variables:
%  output:
...
warning('off','all');
while 1
   [cmat,xS,ef]=cs_CheckAntiBal(n,S,cs);
   if isempty(S)
      warning('on','all');
      bcQ=false;
      break;
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [cmat,sol,ef]=cs_CheckAntiBal(n,iS,cs)
