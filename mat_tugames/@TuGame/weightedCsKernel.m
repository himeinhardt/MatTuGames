function [x, Lerr, smat, xarr]=weightedCsKernel(clv,cs,pS,x)
% WEIGHTEDCSKERNEL computes from (v,pS,x) a weighted kernel element w.r.t. 
% coalition structure cs using
% Matlab's Optimization Toolbox.
% Source: Meinhardt, 2010.
%
% Usage: [x Lerr smat xarr] = clv.weightedCsKernel(cs,pS,x)
%
% Define variables:
%  output:
%  x        -- A weighted kernel element (output) w.r.t. cs.
%              (default per capita kernel).
%  Lerr     -- List of computed function values of hx and h. 
%  smat     -- A matrix of weighted maximum surpluses.
...
while cnt<CNT  
    cnt=cnt+1;
    [A, smat,ei,ecs]=cs_effCoalitions(v,x,smc,cnt,pS,cs);
    if length(cs)>1
       B12=-inf(n,n);
       B21=-inf(n,n);
       cssm=diag(ei);
...
%--------------
function [A, smat,ei,ecs]=cs_effCoalitions(v,x,smc,cnt,clS,cs)
