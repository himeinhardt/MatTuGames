function [x, Lerr, smat, xarr]=cs_PreKernel(clv,cs,x)
% CS_PREKERNEL computes from (v,x) a pre-kernel element of game v w.r.t. the coalition structure cs.
%
% Usage: [x Lerr smat xarr]=clv.cs_PreKernel,cs,x)
%
% Define variables:
%  output:
%  x        -- Pre-Kernel element (output) of game v w.r.t cs.
...
x=x';
[x, Lerr, smat, xarr]=cs_computePrk(v,x,smc,0,mnQ,cs);
smat=tril(smat,-1)+triu(smat,1);

% Main function to compute a
% pre-kernel element.
%-----------------------------
function [x, Lerr, smat, xarr]=cs_computePrk(v,x,smc,slv,mnQ,cs)
% 
%  output:  -- as above.
%
%  input:
...
if cnt==CNT, % should trigger errors ....
  ...
  elseif slv==1 && smc==0 
       ...
       [x, Lerr, smat, xarr]=cs_computePrk(v,x,1,1,mnQ,cs);
  elseif slv==1 && smc==1
       if mnQ==1 && n < 8 
       ...
       [x, Lerr, smat, xarr]=cs_computePrk(v,x,2,1,mnQ,cs);
  else
       x=x';
