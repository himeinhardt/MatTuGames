function [x, Lerr, smat, xarr]=p_cvx_weightedKernel(clv,pS,x)
% P_CVX_WEIGHTEDKERNEL computes from (v,pS,x) a weighted kernel element using CVX
% and Matlab's PCT.
% Source: Meinhardt, 2010.
% 
% CVX Research, Inc. CVX: Matlab software for disciplined convex 
% programming, version 2.0 beta. http://cvxr.com/cvx, September 2012. 
%
% CVX-Solver: http://cvxr.com/cvx/download/
%
% Usage: [x Lerr smat xarr] = p_cvx_weightedKernel(clv,pS,x)
%
% Define variables:
%  output:
...
%cvx_optval 
    if cvx_optval == -Inf  
       x=x';
       warning('kerA:No','Probably no kernel point found!');
       break;
    elseif cvx_optval == Inf
       x=x';
       warning('kerB:No','Probably no kernel point found!');
       break;
    elseif abs(cvx_optval -ofval)<tol
       if irQ
          smat=tril(smat,-1)+triu(smat,1);
          krm=smat-smat';
          irm=repmat(ir,n,1);
...
%  warning('kerC:No','Probably no kernel point found!');
%  x=x';
end
