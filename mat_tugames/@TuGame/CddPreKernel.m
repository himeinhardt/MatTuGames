...
xarr=[];;
for i=1:CNT
  [x, Lerr01, smat, xarr01]=SeekPrk(v,x,1,n,N,i,gt);
  Lerr=[Lerr; Lerr01];
  xarr=[xarr; xarr01];
end
...
if all(all(abs(smat-smat')<2*10^6*eps))==1,
 ...
 warning(msg01);
 for i=1:CNT
   [x, Lerr01, smat, xarr01]=SeekPrk(v,x,0,n,N,i,gt);
   Lerr=[Lerr; Lerr01];
   xarr=[xarr; xarr01];
 end
x=x';
...
%-----------------------------------------
function [x, Lerr, smat, xarr]=SeekPrk(v,x,smc,n,N,cnt,gt)
%
%
...
[Ac, smat]=effCoalitions(v,x,smc,cnt,gt);
upe=triu(upe,1);
ec12=Ac(upe)';
etr21=Ac';
ec21=(etr21(upe))';
