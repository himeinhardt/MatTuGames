function [x, Lerr, smat, xarr]=p_weightedAnti_PreKernel(clv,pS,x)
% WEIGHTEDANTI_PREKERNEL computes from (v,pS,x) a weighted anti-pre-kernel element using Matlab's PCT.
% Source: Meinhardt, 2010.
%         Funaki and Meinhardt, 2006.
%
% Usage: [x Lerr smat xarr]=p_weightedAnti_PreKernel(clv,pS,x)
%
% Define variables:
%  output:
%  x        -- A weighted anti pre-kernel element (output)
%              (default per capita anti pre-kernel).
%  Lerr     -- List of computed function values of hx and h. 
%  smat     -- Matrix of minimum weighted surpluses.
...
[x, Lerr, smat, xarr]=computePrk(v,x,pS,smc,0,gt);
smat=tril(smat,-1)+triu(smat,1);

% Main function to compute a
% pre-kernel element.
%-----------------------------
function [x, Lerr, smat, xarr]=computePrk(v,x,pS,smc,slv,gt)
% 
%  output:  -- as above.
%
%  input:
...
if isempty(pS)
  S=1:N;
  mat=zeros(n,N);
  parfor k=1:n, mat(k,:) = bitget(S,k);end
  clS=ones(1,n)*mat;
...
while cnt<CNT  
    cnt=cnt+1;
    [A, smat]=effCoalitions(v,x,smc,cnt,gt,pS);
    upe=tril(upe,-1);
    etr12=A';
    ec12=etr12(upe)';
    ec21=A(upe)';
...
if cnt==CNT, % should trigger errors ....
  if slv==0 && smc==1
       ...
       warning('APrK:ChangCard',msg01);
       [x, Lerr, smat, xarr]=computePrk(v,x',pS,0,slv,gt);
  elseif slv==0 && smc==0 
       msg02='No Pre-Kernel Element found. Changing the Solver.';
       warning('APrK:ChangSolv',msg02);
       [x, Lerr, smat, xarr]=computePrk(v,x',pS,smc,1,gt);
  elseif slv==1 && smc==0 
       msg01='No Pre-Kernel Element found. Changing Cardinality to Default Value.';
       warning('APrK:Default',msg01);
       [x, Lerr, smat, xarr]=computePrk(v,x',pS,1,1,gt);
  elseif slv==1 && smc==1
       x=(v(N)/n)*ones(1,n);
       msg01='No Pre-Kernel Element found. Changing to Start Value.';
       warning('APrK:StartVal',msg01);
       [x, Lerr, smat, xarr]=computePrk(v,x,pS,2,1,gt);
  else
       x=x';
       msg02='No Pre-Kernel Element found. Change payoff vector and restart!';
       warning('APrK:NotFound',msg02);
  end
else
  x=x';
end
...
%--------------
function [A, smat]=effCoalitions(v,x,smc,cnt,gt,clS)
