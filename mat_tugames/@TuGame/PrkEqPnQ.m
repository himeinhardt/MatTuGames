...
%  ptn      -- Returns the set of partitions which are included in D(x), the set of maximal excesses.
...
if nargin<2
   tol=10^6*eps;
   x=clv.PreKernel(); %% Taking an arbitrary pre-kernel element.
elseif nargin<3
   tol=10^6*eps;
end
N=clv.tusize;
...
if SATIS.ptnQ==0
  ...
  for k=1:sz(2)
      sS=ptn(1,k);
      inS=bitget(sS,pl)==1;
      v_t{1,k}=clv.RedGame(x,sS);
      v_tcv{1,k}=clv.CvRedGame(x,sS);
