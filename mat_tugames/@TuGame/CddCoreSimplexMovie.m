function CddCoreSimplexMovie(varargin)
...
%
% Usage:       CddCoreSimplexMovie(v) 
%                    which is the default  usage. 
%              CddCoreSimplexMovie(v,'all','3','2.sec','zoom','jpg')
...
%                Then invoke
%
%                CddCoreSimplexMovie(v,'all',vw_pt)
%
...
t(end+1)=fmin;
if crQ==1
  [crv_vert,~,~,Pv]=clv.CddCoreSimplexVertices;
else
  vfmin=clv.streps_value(fmin);
  [crv_vert,~,~,Pv]=CddCoreSimplexVertices(vfmin);
end
[imp_vert,~,~,Pip]=clv.CddImputationSimplexVertices;
v_prk=clv.PreKernel;
...
%   for k=1:8
    ...
    [cr_eps{k},crst_eps{k},vol_eps(k),P(k)]=CddCoreSimplexVertices(v_eps(k,:));
    ms2=min(P(1).V);
    ml2=max(P(1).V);
    mrg=min(max(ml2)/4,2);
