function [x, Lerr, smat, xarr]=p_clp_weightedKernel(clv,pS,x)
% P_CLP_WEIGHTEDKERNEL computes from (v,pS,x) a weighted kernel element using mexclp
% and Matlab's PCT.
% Source: Meinhardt, 2010.
%
% CLP-Solver: https://projects.coin-or.org/Clp 
%
% Usage: [x Lerr smat xarr] = p_clp_weightedKernel(clv,pS,x)
%
% Define variables:
%  output:
...
% Calling quadratic programming solver.
    ...
    [x,dz,exitflag] = clp(Q,b,[],[],E(m,:),a(m),vi,ra,options);
    if exitflag ~= 0 
       x=x';
