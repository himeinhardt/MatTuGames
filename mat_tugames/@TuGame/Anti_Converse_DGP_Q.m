function [ACDGP ACDGPC]=Anti_Converse_DGP_Q(clv,x,str,tol)
...
sV_x{1,k}=x;
 if strcmp(str,'SHAP')
   vS{1,k}=clv.HMS_Anti_Derived_game(x,cl2(k)); %Hart-MasColell anti-derived game.
   ...
   vS{1,k}=clv.Anti_DerivedGame(x,cl2(k)); % Davis-Maschler anti-derived game.
   stdsol{1,k}=StandardSolution(vS{1,k}); % solution x restricted to S.
   rS{k}=PlyMat2(k,:);
   sV_x{1,k}(rS{k})=stdsol{1,k}; % extension to (x,x_N\S).
   if strcmp(str,'MAPRK')
     crgpQ(k)=clv.Anti_ModPrekernelQ(sV_x{1,k});
   elseif strcmp(str,'PMAPRK')
     crgpQ(k)=clv.Anti_PModPrekernelQ(sV_x{1,k});
   elseif strcmp(str,'MODIC')
     crgpQ(k)=clv.modiclusQ(sV_x{1,k});
   elseif strcmp(str,'APRK')
     crgpQ(k)=clv.Anti_PrekernelQ(sV_x{1,k});
