...
[x, Lerr, smat, xarr]=computePrk(v,x,smc,0,mnQ,gt);
smat=tril(smat,-1)+triu(smat,1);

% Main function to compute a
% pre-kernel element.
%-----------------------------
function [x, Lerr, smat, xarr]=computePrk(v,x,smc,slv,mnQ,gt)
% 
%  output:  -- as above.
%
%  input:
...
if cnt==CNT, % should trigger errors ....
  if slv==0 && smc==1
       ...
       [x, Lerr, smat, xarr]=computePrk(v,x',0,slv,mnQ,gt);
  elseif slv==0 && smc==0 
       msg02='No Pre-Kernel Element found. Changing the Solver.';
       warning('PrK:ChangSolv',msg02);
       if mnQ==1; x=2*x; end  
       [x, Lerr, smat, xarr]=computePrk(v,x',smc,1,mnQ,gt);
  elseif slv==1 && smc==0 
       ...
       [x, Lerr, smat, xarr]=computePrk(v,x',1,1,mnQ,gt);
  elseif slv==1 && smc==1
       x=(v(N)/n)*ones(1,n);
       msg01='No Pre-Kernel Element found. Changing to Start Value.';
       warning('PrK:StartVal',msg01);
       [x, Lerr, smat, xarr]=computePrk(v,x,2,1,mnQ,gt);
  else
       x=x';
