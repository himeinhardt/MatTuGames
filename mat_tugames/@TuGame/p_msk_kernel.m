function [x, Lerr, smat, xarr]=p_msk_kernel(clv,x)
% P_MSK_KERNEL computes from (v,x) a Kernel element using qpas.mex
% and Matlab's PCT.
% Source: Meinhardt, 2010.
%
% MSK-Solver: http://www.mosek.com/  
%
% Usage: [x Lerr smat xarr] = p_msk_kernel(clv,x)
%
% Define variables:
%  output:
...
% Calling conic programming solver.
    ...
    prob.c=c;
    prob.blx=sparse([vi;-inf(n+1,1)]);
    prob.bux=sparse([ra;inf(n+1,1)]);
