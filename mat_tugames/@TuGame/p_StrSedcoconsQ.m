function [SCOSED SCOSEDC]=p_StrSedcoconsQ(clv,x,str,tol)
...
%
%
% Usage: [SCOSED SCOSEDC]=clv.p_StrSedcoconsQ(x,str,tol)
%
% Define variables:
%  output: Fields
...
if strcmp(str,'MAPRK')
  sV_sol{1,N}=clv.p_Anti_ModPreKernel(x);
elseif strcmp(str,'PMAPRK')
  sV_sol{1,N}=clv.p_Anti_PModPreKernel(x);
elseif strcmp(str,'MODIC')
  try
...
elseif strcmp(str,'APRK')
  sV_sol{1,N}=clv.p_Anti_PreKernel(x);
elseif strcmp(str,'APRN')
  try
    sV_sol{1,N}=clv.cplex_AntiPreNucl_llp();
  catch
    sV_sol{1,N}=clv.Anti_PreNucl_llp(); % use a third party solver instead!
  end
elseif strcmp(str,'SHAP')
  sV_sol{1,N}=clv.p_ShapleyValue;
else
  sV_sol{1,N}=clv.p_ModPreKernel(x);
end
