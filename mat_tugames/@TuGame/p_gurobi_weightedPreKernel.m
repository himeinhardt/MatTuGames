function [x, Lerr, smat, xarr]=p_gurobi_weightedPreKernel(clv,pS,x)
% P_GUROPI_WEIGHTEDPREKERNEL computes from (v,pS,x) a weighted pre-kernel element using gurobimex
% and Matlab's PCT
...
%
%
% Usage: [x Lerr smat xarr] = p_gurobi_weightedPreKernel(clv,pS,x)
%
% Define variables:
%  output:
...
% Calling quadratic programming solver.
    ...
    if strcmp(result.status,'INFEASIBLE')
       x=x';
       warning('Pkr:NoA','Probably no pre-kernel point found!');
       break;
    elseif strcmp(result.status,'OPTIMAL') ~= 1 
       x=x';
       warning('Pkr:NoB','Probably no pre-kernel point found!');
       break;
    end
...
%  warning('Pkr:No','Probably no pre-kernel point found!');
%  x=x
end
