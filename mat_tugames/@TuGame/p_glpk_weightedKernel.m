function [x, Lerr, smat, xarr]=p_glpk_weightedKernel(clv,pS,x)
% P_GLPK_WEIGHTEDKERNEL computes from (v,pS,x) a weighted kernel element using glpkmex
% and Matlab's PCT.
...
%
%
% Usage: [x Lerr smat xarr] = p_glpk_weightedKernel(clv,pS,x)
%
% Define variables:
%  output:
...
%  smat     -- Matrix of maximum weigthed surpluses.
...
% Calling quadratic programming solver.
    [x, obj,~, info] = qpng(Q,b,C,c,ctype,vi,ra,x);
%     [x, obj,~, info] = qpng(Q,b,E(m,:),a(m),'E',vi,ra,x);
    if info.status ~= 1
       x=x';
