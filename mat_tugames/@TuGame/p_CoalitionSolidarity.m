function cs_vl=p_CoalitionSolidarity(clv,cs)
% P_CoalitionSolidarity computes a coalitional solidarity value w.r.t. 
% the coalition structure cs using Matlab's PcT.    
%
% Usage: cs_vl=p_CoalitionSolidarity(clv,cs)
%
% Define variables:
%  output:
%  cs_vl    -- A coalitional solidarity value for coalition structure cs.
%
%  input:
      ...
      if lcm > 1
         ...
         for l=1:lcm
           ...
           for i=1:cbd
              ...
              sd_av(i,:)=ShapleyValue(av);
           end
         end
         % The reduced game. 
         v1=[sd_av(:,jj)',sh_cw(jj)];
         ics{jj}=SolidarityValue(v1);
      else    
         ics{jj}=sh_cw(jj);
      end
  end
