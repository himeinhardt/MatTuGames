function [x, Lerr, smat, xarr]=p_weightedPreKernel(clv,pS,x)
% P_WEIGHTEDPREKERNEL computes from (v,pS,x) a weighted pre-kernel element using Matlab's PCT.
% Inspired by Jean Derks, see email from 26/05/2010.
% Source: Meinhardt, 2010.
%
% Usage: [x Lerr smat xarr]=p_weightedPreKernel(clv,pS,x)
%
% Define variables:
%  output:
...
% Q is symmetric and positive definite. We apply a Cholesky decomposition.
    ...
    else x=qrginv(Q)*b; % (SVD-decomposition)
    end
   else
     x=qrginv(E)*a;
   end
