function [x, Lerr, smat, xarr]=cfr_Anti_PreKernel(clv,F,x)
%CFR_ANTI_PREKERNEL computes from (v,x) an anti-pre-kernel element 
% with coalition formation restrictions. 
%
% Usage: [x Lerr smat xarr]=clv.cfr_Anti_PreKernel(F,x)
%
% Define variables:
%  output:
%  x        -- Anti-Pre-Kernel element (output) w.r.t. F.
...
F=unique([F,si]);
[x, Lerr, smat, xarr]=cfr_computeAntiPrk(v,x,smc,0,mnQ,F);
smat=tril(smat,-1)+triu(smat,1);

% Main function to compute a
% pre-kernel element.
%-----------------------------
function [x, Lerr, smat, xarr]=cfr_computeAntiPrk(v,x,smc,slv,mnQ,F)
% 
%  output:  -- as above.
%
%  input:
