function ow_vl=p_BanzhafOwenValue(clv,cs)
% P_BANZHAFOWEN_VALUE computes the Banzhaf-Owen value w.r.t. 
% a priori unions cs using Matlab's PCT.
%
% Usage: ow_vl=clv.p_BanzhafOwenValue(cs)
%
% Define variables:
%  output:
%  ow_vl    -- The Banzhaf-Owen value (normalized to one).
%
%  input:
      ...
      if lcm > 1
         ...
         for l=1:lcm
           ...
           for i=1:cbd
              ...
              sh_av(i,:)=banzhaf(av); % normalized to one.
	      if any(isnan(sh_av(i,:))) % game with no winning coalition.
		 sh_av(i,:)=zeros(1,lcs);
       	      end
           end
         end
