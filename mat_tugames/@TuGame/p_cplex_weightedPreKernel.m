function [x, Lerr, smat, xarr]=p_cplex_weightedPreKernel(clv,pS,x)
% P_CPLEX_WEIGHTEDPREKERNEL computes from (v,pS,x) a weighted pre-kernel element using cplexmex
% and Matlab's PCT.
% Source: Meinhardt, 2010.
%
% CPLEX-Solver: http://www-01.ibm.com/software/websphere/ilog/
%
%
% Usage: [x Lerr smat xarr] = p_cplex_weightedPreKernel(clv,pS,x)
%
% Define variables:
%  output:
...
while cnt<CNT  
    ...
    a(m)=v(N);
    if n==2, a=a'; end;
    err=norm(E*x-a)^2; if err<10^7*eps, x=x';break; end
    Q=2*E'*E;
    b=-2*E'*a;
...
warning('on','all');
    if slv==0
       [x,fval,exitflag,output,lambda] = cplexqp(Q,b,[],[],E(m,:),a(m),[],ra,x,options);
    elseif slv==1 
       [x, resnorm, residual, exitflag, output, lambda] = cplexlsqlin(E,a,[],[],E(m,:),a(m),[],ra,x,options);
    else
       [x,fval,exitflag,output,lambda] = cplexqp(Q,b,[],[],E(m,:),a(m),[],ra,x,options);  
    end
    if exitflag ~= 1
