function [cow_vl vr]=ColemanOwenValue(clv,cs)
% COLEMANOWEN_VALUE computes the Coleman-Owen value w.r.t. 
% a priori unions cs.
%
% Usage: cow_vl=clv.ColemanOwenValue(cs)
%
% Define variables:
%  output:
%  cow_vl    -- The Coleman-Owen value.
%
%  input:
...
cw=cw(pcl2);
sh_cw=BanzhafColeman(cw); % normalized Banzhaf value by 2^lcs-1.
cow_vl=zeros(1,n);
J=1:n;
int=0:-1:1-n;
csm=rem(floor(cs(:)*pow2(int)),2)==1;
  for jj=1:lcs
      ...
      if lcm > 1
         ...
         cow_vl([idx])=ShapleyValue(v1);
      else    
         cow_vl([idx])=sh_cw(jj);
      end
  end
else
   cow_vl=ShapleyValue(v); 
end  
%-----------------------------------------
