...
y1=range(impv);
[~,idx]=min(y1);
[imp_vol,Pip]=Projection(impv,idx,n);
% Core
if core_sol==1
  [cr_vol,Pv]=Projection(crv_vert,idx,n);
end
[~,~,rs_vol,Prea]=clv.CddReasonableSetVertices(idx);
[~,~,prs_vol,Pprea]=clv.CddUpperSetVertices(idx);
sma=clv.smallest_amount();
lwseQ=sum(sma)>v(N);
if lwseQ==1
else
  [~,~,lws_vol,Plws]=clv.CddLowerSetVertices(idx);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Pvol,P]=Projection(cr_v,idx,n)   
%
%
if n>3
   if isempty(idx)
