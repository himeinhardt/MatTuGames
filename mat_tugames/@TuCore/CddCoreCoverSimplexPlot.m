function CddCoreCoverSimplexPlot(varargin)
% CDDCORECOVERPLOT plots the core cover set of game v. 
%
% Requires the Multi-Parametric Toolbox 3
% http://people.ee.ethz.ch/~mpt/3/
%
% Usage: CddCoreCoverPlot(varargin)
% Define variables:
%  output:
%             -- A plot of the Core Cover set of game v.
%
...
%  core_sol   -- An integer to draw the core cover set in connection with the core.
%                The core will be drawn if the size is different from the 
%                Core Cover set, otherwise it is ommitted.
...
%                Then invoke
%
%                CddCoreCoverSimplexPlot(v,'all',vw_pt)
%
...
if core_sol==1
  crv_vert=ToSimplex3d(crv_vert);
  Pv=Polyhedron(crv_vert);
  cr_vol=volume(Pv);
end
impv=ToSimplex3d(impv);
Pip=Polyhedron(impv);
ms1=min(Pip.V);
ml1=max(Pip.V);
