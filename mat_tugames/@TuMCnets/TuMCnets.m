classdef TuMCnets < TuGame
% TUMCNETS is a subclass object of TUGAME to perform several computations for retrieving 
% and modifying game data. It stores relevant game information and solutions needed by
% overloading functions.
%
% Usage: clv = TuMCnets(struc,n,'gtype','gformat')
%
% Define variables:
% output:
% clv           -- TuMCnets class object (subclass of TuGame).
%
% input:
...
%
% TuMCnets properties:
%  tu_strc      -- stores the rules set in form of a structure array.
...
%
% TuMCnets methods:
%  TuMCnets         -- creates the class object TuMCnets.
%  setAllSolutions  -- sets all solutions listed below to the class object TuMCnets. 
%  setPreKernel     -- sets a pre-kernel element to the class object TuMCnets.
%  setPreNuc        -- sets the pre-nucleolus to the class object TuMCnets.
%  setShapley       -- sets the Shapley value to the class object TuMCnets.
%  setTauValue      -- sets the Tau value to the class object TuMCnets.
%  setBanzhaf       -- sets the Banzhaf value to the class object TuMCnets.
%  setAntiPreKernel -- sets an anti-pre-kernel element to the class object TuMCnets.
%  
...
%
    ...
    methods
       function obj = TuMCnets(strc,n,gtype,gformat)
       % TUMCNETS creates the subclass object TuMCnets to perform several computations for retrieving 
       % and modifying game data. It stores relevant game information and solutions needed by
       % overloading functions.
       %
       % Usage: clv = TuMCnets(strc,n,'gtype','gformat')
       %
       % Define variables:
       % output:
       % clv           -- TuMCnets class object (subclass of TuGame).
       %
       % input:
       % strc          -- Rules set represented as a structure / cell array.
       % n             -- Specifies the number of players involved in the game, must be an integer.
       % gtype         -- A string to define the game type.
       %                    Permissible types are:
       %                    -- 'cv' (convex/average-convex, semi-convex).
       ...
       %
           ...
           elseif nargin < 2
              error('MCnets information must be given as a structure or cell array with number of players!');
           elseif nargin < 3 
              gtype = '';
              gformat = 'mattug';
           elseif nargin < 4
	   ...
	   if iscell(strc)
              strc = GetMCNetRules(strc);
           end
	   w = ReverseMCNetsRep(strc,n);
           obj@TuGame(w,gtype,gformat);
	   obj.tu_strc=strc;
       end
        ...
        function obj = set.tu_aprk(obj,pt)
         ...
         % SETALLSOLUTIONS sets a set of game solutions to the class object TuMCnets.
         %
