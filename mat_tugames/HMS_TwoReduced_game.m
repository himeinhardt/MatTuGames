function [v_t,sS,PlyMat2]=HMS_TwoReduced_game(v,x,str,tol)
...
for k=1:lgt
 ...
 elseif strcmp(str,'CORE')
   ...
   y=x(pl);
   try
      crQ=CddCoreQ(subg{k},tol);	   
   catch	   
      crQ=coreQ(subg{k},tol);
   end   
   if crQ==1	   
	 ...
	 if convex_gameQ(subg{k})	 
	   %      mgc=AllMarginalContributionsResToS(v,TorcS(k));
           %      umgc=unique(mgc,'row')
           subg_sh{k}=FindHMConsElm(subg{k},y,TorcS(k),S,n,lmQ); % try to get HM-consistent core element.
	 else
            %subg_sh{k}=FindHMConsElm(subg{k},y,TorcS(k),S,n,lmQ);		 
