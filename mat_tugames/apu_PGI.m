function [apgi vr]=apu_PGI(v,cs)
% APU_PGI computes the Holler index w.r.t. 
% a priori unions cs.
%
% Usage: apgi=apu_PGI(v,cs)
%
% Define variables:
%  output:
...
%% PGI for the quotient game.
cw=cw(pcl2);
J=1:n;
int=0:-1:1-n;
mWC=getMinimalWinning(cw);
csm=rem(floor(cs(:)*pow2(int)),2)==1;
for k=1:lcs;
    mWCk=mWC(bitget(mWC,k)==1);
    sWCk(k)=length(mWCk)*nnz(csm(k,:));
end
sh_cw=sWCk./(sWCk*ones(lcs,1));
apgi=zeros(1,n);
  for jj=1:lcs
      Tj=csm(jj,:);
      idx=J(Tj);
