function v=ReverseMCNetsRep(strc,n)
...
%
%
%  Usage: v=ReverseMCNetsRep(strc,n)
%
%    
% Define variables:
%  output:
...
%
% Then translating the ruleset into a structure array through 
%    
...
for ii=1:s
    zv=zeros(1,N);
    cl=clToMatlab(strc(ii).PositiveLiterals);
    sps=SuperSets(cl,n);
    ncl=clToMatlab(strc(ii).NegativeLiterals);
    if isempty(ncl)==1 && isempty(cl)==0 
       zv(sps)=strc(ii).Value;
       v=v+zv;
    elseif isempty(ncl)==0 && isempty(cl)==0 
       nsps=sS(ismember(sS,SuperSets(ncl,n))==0);
       sps2=sps(ismember(sps,nsps));
       sps2=sps2(bitand(sps2,ncl)==0); % No intersection of sets is allowed. Removing intersections. 
       zv(sps2)=strc(ii).Value;
       v=v+zv;
    elseif isempty(ncl)==0 && isempty(cl)==1
       cmpl=N-ncl; % taking the complement of the negative literals. 
       sups=SubSets(cmpl,n);
       zv(sups)=strc(ii).Value;
       v=v+zv;
    end   
end    