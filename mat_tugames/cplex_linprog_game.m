function [v,x]=cplex_linprog_game(A,H,mB,mD,p)
...
%
% Usage: [v,x]=cplex_linprog_game(A,H,mB,md,p)
%
% Define variables:
% output:
...
if isempty(H)==1 && isempty(A)==0
   for S=1:N
       ...
       B=adm(:,S)';
       [xmax, fmax, status, extra] = cplexlp(C,A,B,[],[],lb,ub,options);
       v(S)=-fmax;
       x(S,:)=xmax';
   end
elseif isempty(H)==0 && isempty(A)==1
  for S=1:N
       % Matrix and boundary vector for coalition S.
       D=ddm(:,S)';
       [xmax, fmax, status, extra] = cplexlp(C,[],[],H,D,lb,ub,options);
       v(S)=-fmax;
       x(S,:)=xmax';
  end    
elseif isempty(H)==1 && isempty(A)==1   
  for S=1:N
       ...
       B=adm(:,S)';
       D=ddm(:,S)';
       [xmax, fmax, status, extra] = cplexlp(C,A,B,H,D,lb,ub,options);
       v(S)=-fmax;
       x(S,:)=xmax';
  end
end 
