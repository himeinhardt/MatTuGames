function [my_vl, vG, SG]=MyersonValue(v,cs,str)
% MYERSON_VALUE computes the Myerson value w.r.t. 
% a communication situation.
%
% Usage: my_vl=MyersonValue(v,cs,str)
%
% Define variables:
%  output:
...
vG=zeros(N,1);
 for k=1:N
     sb=SubSets(k,n);
     cs=sort(cs);
%     iS=sb(builtin('_ismemberoneoutput',sb,cs));
     iS=sb(ismembc(sb,cs));
