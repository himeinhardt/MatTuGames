function [x1, fmin]=cs_Anti_Nucl(v,cs,tol)
...
%
% Usage: [x, fmin]=cs_Anti_Nucl(v,cs,tol)
% Define variables:
%  output:
...
k=1:n;
ra = smallest_amount(v); %% Cannot be used as an lower bound. Does not recognize coalition structure.
Nk=N-2.^(k-1);
vi=v(Nk);
%vi=v(bitset(0,k));
%vnk=v(Nk);
