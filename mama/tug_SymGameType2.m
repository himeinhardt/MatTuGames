function SOL=tug_SymGameType2(n,S,val)
% TUG verifies game solution/property with the Mathematica Package TuGames.
%
% Usage: SOL=tug_SymGameType2(n,S,val)
%  Example: SOL=tug_SymGameType2(5,22,4)
% Define variables:
%  output:
...
if nargin<1
    ...
    val=3;
elseif nargin<2
    error('A subcoalition of size not less than 2 must be given!');
    val=3;
else
end
...
elseif nnz(sS)<2
   error('The sub-coalition has size smaller than 2!');
else
end
...
disp('Determing Symmetric Game Type 2 ...')
mabv=math('gsym=SymGameType2[T,S2,mv]');
