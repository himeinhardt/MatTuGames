function SOL=tug_ContestedGarment(E,d)
% TUG verifies game solution/property with the Mathematica Package TuGames.
%
% Usage: SOL=tug_ContestedGarment(E,d)
% Define variables:
%  output:
%  SOL        -- A division rule of a contested garment problem of size 2.
%                Field variable gives result in Matlab and Mathematica format.
%  input:
%  E          -- Estate, it is an integer value.
%  d          -- Claimants vector of length (1x2).
%
...
elseif nargin<2
  ...
  if length(d)~=2
     error('The claims vector must have lenght 2!'); 
  else
  end
  if length(E)~=1
...
mcgr=math('cgr=ContestedGarment[est1,clv]');
cg_rl=math('math2matlab','cgr');
SOL=struct('ContestedGarment',cg_rl,'MContestedGarment',mcgr);
math('quit')
