function SOL=tug_SymGameType4(n,S,val)
% TUG verifies game solution/property with the Mathematica Package TuGames.
%
% Usage: SOL=tug_SymGameType4(n,S,val)
%  Example: SOL=tug_SymGameType4(5,30,4)
% Define variables:
%  output:
...
if nargin<1
    error('The number of players for the player set must be specified! It should have at least 5-persons!');
    S=7;
    val=3;
elseif nargin<2
    error('A subcoalition of size not less than 4 must be given!');
    val=3;
else
%    val=3;
end
...
elseif nnz(sS)<4
   error('The sub-coalition has size smaller than 4!');
end
...
disp('Determing Symmetric Game Type 4 ...')
mabv=math('gsym=SymGameType4[T,S2,mv]');
